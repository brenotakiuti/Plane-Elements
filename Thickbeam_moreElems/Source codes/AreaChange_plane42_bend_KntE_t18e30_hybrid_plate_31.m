...
Ndof = 2;
b = 5e-3;              % base of the cross-section (tickness) (m)
...
NeleB = round(L/Lb);            % Number of elements in B
Sa = b*(ha);
Sb = b*hb;
Sc = b*hc;
Ia = b*(ha)^3/12;
...
for q=1:length(f)
    ...
    [ PhiQp(:,:,q),PhiQn(:,:,q),PhiFp(:,:,q),PhiFn(:,:,q),PsiQp(:,:,q),PsiFp(:,:,q),...
      ...
      = PolySolve_complex6( w(q),Ka,Ma,La,nor,tol,lim,lim2);
...
%     [ PhiQb_p,PhiQb_n,PhiFb_p,PhiFb_n,PsiQb_p,PsiFb_p,PsiQb_n,PsiFb_n,lpb1,lnb1,sb,kpbi,knbi ] = PolySolve_complex( w,Kb,Mb,Lb,nor,tol);
    [ PhiQp(:,:,q),PhiQn(:,:,q),PhiFp(:,:,q),PhiFn(:,:,q),PsiQp(:,:,q),PsiFp(:,:,q),...
      PsiQn(:,:,q),PsiFn(:,:,q),lp(:,q),ln(:,q),s(:,q),kp(:,q),kn(:,q),~,PureN ] ...
      = PolySolve_complex6( w(q),Kb,Mb,Lb,nor,tol,lim,lim2); %for 30-10 cases, use lim1 and lim2 default
end
...
for q=1:length(f)
    ...
    [ PhiQp(:,:,q),PhiQn(:,:,q),PhiFp(:,:,q),PhiFn(:,:,q),PsiQp(:,:,q),PsiFp(:,:,q),...
      PsiQn(:,:,q),PsiFn(:,:,q),lp(:,q),ln(:,q),s(:,q),kp(:,q),kn(:,q),~,PureN ]...
      = PolySolve_complex6( w(q),Kc,Mc,Lc,nor,tol,lim,lim2);
